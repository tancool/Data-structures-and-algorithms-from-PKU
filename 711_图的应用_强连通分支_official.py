...
class DFSGraphStrongConnect(Graph): # 强连通分支类.继承自Graph
    ...
    def dfs(self): # 执行深度优先
        for v in self:
            v.setColor('white')
            v.setPred(None)
        lst = list(self.vertices.values()) # 存储为数组.(引用)
        lst.sort(key=lambda v:-v.finishTime) # 数组按照 finishTime递减排序
...
# -------------------------以上是深度优先-----------------------------
    ...
    for node in g: # 遍历深度优先的函数. 遍历节点
        for nbr in node.connectedTo.keys(): # 遍历邻接表
            ...
            gt.addEdge(nbr.getId(),node.getId()) # 添加边(反向添加) / 复制的值,而非引用

            # -------------------发现时间和完成时间都是不变的,仅仅是改变了指向-------------------------
            
            start = gt.getVertex(nbr.getId()) # 如果此节点存在.返回此节点
            start.discoveryTime = nbr.getDiscovery() # 发现时间不变
            start.finishTime = nbr.getFinish() # 完成时间不变

            end = gt.getVertex(node.getId()) # 判断此节点是否存在
            end.discoveryTime = node.getDiscovery() # 赋值发现时间
            end.finishTime = node.getFinish() # 赋值完成时间


    print('-'*70)

    for node in gt: # 通过这行代码可以知道 发现时间和完成时间没有改变,仅仅是改变了有向图的指向
        print(node.getId(),node.getDiscovery(),node.getFinish())

    print('-' * 70)

    mydict = gt.dfs() # 执行深度优先
    for node in gt:
        print(node.getId(),node.getDiscovery(),node.getFinish())
    print('-' * 70)
